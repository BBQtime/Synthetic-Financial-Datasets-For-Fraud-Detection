...
print('\n OLD BALANCE PROBLEM in victim account is {}'.format(len(X.loc[(y==1)&(X['amount']==0)])))

print('\n OLD BALANCE PROBLEM in victim account is {}'.format(len(X.loc[(X['Old_Balance_Orig']<X['amount'])&(y==0)])))

print('\n OLD BALANCE PROBLEM in victim account is {}'.format(len(X.loc[(X['Old_Balance_Orig']<X['amount'])&(y==1)])))

print('\n OLD NEW AMOUT ARE ALL ZERO IN FRAUD IS {}'.format(
    len(X.loc[(X['Old_Balance_Orig']==0) & (X['amount']==0) &(X['New_Balance_Orig']==0) &(y==1)])))

print('\n OLD NEW AND AMOUNT ARE ALL ZERO is {}'.format(
    len(X.loc[(X['Old_Balance_Orig']==0) & (X['amount']==0) &(X['New_Balance_Orig']==0) &(y==0)])))
